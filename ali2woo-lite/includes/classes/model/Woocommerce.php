...
class Woocommerce
{
    ...
    public function build_steps($product)
    {
        ...
        if ($images_data['thumb']) {
            $steps[] = 'images#' . Utils::buildImageIdFromPath($images_data['thumb']);
        }
        foreach ($images_data['images'] as $image_url) {
            $steps[] = 'images#' . Utils::buildImageIdFromPath($image_url);
        }
    ...
    private function prepare_product_images($product)
    {
        ...
        if ((!$override_product || $override_images) && isset($product['images'])) {
            ...
            foreach ($product['images'] as $image) {
                if (!in_array(Utils::buildImageIdFromPath($image), $product['skip_images'])) {
                    $image_to_load[Utils::buildImageIdFromPath($image)] = $image;
                }
            }
    ...
    public function add_product($product, $params = array()): array
    {
            ...
            if ($override_product && $this->is_product_exist($product_id)) {
                ...
                foreach ($_product_attributes as $attr_key => $attr) {
                    if (!empty($attr['is_variation'])) {
                        // unlink attr values
                        $delete_query = "DELETE tr FROM {$wpdb->term_relationships} tr " .
                                        "INNER JOIN {$wpdb->term_taxonomy} tt on (tt.term_taxonomy_id=tr.term_taxonomy_id) " .
                                        "WHERE tr.object_id=%d and tt.taxonomy=%s";
                        $wpdb->query(
                            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                            $wpdb->prepare($delete_query, $product_id, $attr_key)
                        );
        ...
        if ($step === false || substr($step, 0, strlen('images')) === 'images') {
            ...
            if (!empty($images_data['thumb']) || !empty($images_data['images'])) {
                $this->set_images(
                    $product, $product_id, $images_data['thumb'],
                    $images_data['images'], true, $post_title, $params
                );
            }
        ...
        if ($step === false || substr($step, 0, strlen('variations')) === 'variations') {
            if ($this->need_import_variations($product, $product_type)) {
                foreach ($product['sku_products']['variations'] as &$var) {
                    $var['image'] = (!isset($var['image']) ||
                        in_array(Utils::buildImageIdFromPath($var['image']), $product['skip_images'])) ? '' : $var['image'];
                }
    ...
    public function upd_product($product_id, $product, $params = array())
    {
        ...
        // update variations
        $shouldImportVariantsOnProductUpdate = !a2wl_check_defined('A2WL_DO_NOT_IMPORT_VARIATIONS') &&
            !$wc_product->is_type('external') &&
            !empty($product['sku_products']['variations']) &&
            count($product['sku_products']['variations']) > 0;

        if ($shouldImportVariantsOnProductUpdate) {
            foreach ($product['sku_products']['variations'] as &$var) {
                $var['image'] = (!isset($var['image']) ||
                    in_array(
                        Utils::buildImageIdFromPath($var['image']),
                        $product['skip_images']
                    )) ? '' : $var['image'];
            }
    ...
    {
        //todo: check class-wc-meta-box-product-images.php _product_image_gallery
        $step = $params['step'] ?? false;

        if ($thumb_url && $thumb_url != 'empty' && (!get_post_thumbnail_id($product_id) || $update)
            && ($step === false || $step === 'images#' . Utils::buildImageIdFromPath($thumb_url))) {

            try {
                $tmp_title = !empty($title) ? $title : null;
                $thumb_id = $this->attachment_model->create_attachment(
                    $product_id,
                    $thumb_url,
                    [
                        'inner_post_id' => $product_id,
            ...
            */
            ...
            if (!$cur_product_image_gallery || $update) {
                ...
                foreach ($images as $image_url) {
                    $cnt++;
                    if ($step === false || $step === 'images#' . Utils::buildImageIdFromPath($image_url)) {
                        if ($image_url == $thumb_url) {
                            continue;
                        }
                        try {
                            $tmp_title = !empty($title) ? ($title . ' ' . $cnt) : null;
                            $new_image_gallery_id = $this->attachment_model->create_attachment(
                                $product_id,
                                $image_url,
                                [
                                    'inner_post_id' => $product_id,
                                    ...
                                    'alt' => $tmp_title,
                                    'edit_images' =>
                                        !empty($product['tmp_edit_images']) ? $product['tmp_edit_images'] : []
                                ]
                            );
    ...
    public function addCategory(string $categoryName, int $parentCategoryId = 0, string $categorySlug = ''): ?int
    {
        ...
        $termData = term_exists(
            $categoryName,
            'product_cat',
            $parentCategoryId === 0 ? null : $parentCategoryId
        );

        if (!is_null($termData)) {

            //return existed category id
    ...
    private function add_variation($product_id, $product, $is_update = false, $params = array())
    {
        ...
        if ($is_update) {
            $wc_product = wc_get_product($product_id);
            $isSimpleProduct = $wc_product->is_type('simple');
        }
        ...
        $disable_add_new_variants = get_post_meta($product_id, '_a2w_disable_add_new_variants', true);
        $on_new_variation_appearance = $disable_add_new_variants ? "nothing" :
            ($params['on_new_variation_appearance'] ?? get_setting('on_new_variation_appearance'));
        $on_not_available_variation = ($params['on_not_available_variation'] ?? get_setting('on_not_available_variation'));
        ...
        foreach ($variations['attributes'] as $key => $attr) {
            ...
            foreach ($attr['value'] as &$val) {
                ...
                if (($step === false || $step === 'variations#attributes') && $attr_image) {
                    ...
                    $old_attachment_id = !empty($old_swatch_type_options[$swatch_id]['attributes'][$swatch_value_id]['image']) ?
                        intval($old_swatch_type_options[$swatch_id]['attributes'][$swatch_value_id]['image']) : 0;
        ...
        if (!empty($old_vids_ids)) {
            $preparedIn = implode(', ', $old_vids_ids);
            $_old_vids = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT p.ID, pm.meta_value as variation_id, p.post_status, pm_stock.meta_value as 'quantity', pm_regular.meta_value as 'regular_price', pm_sale.meta_value as 'price' FROM $wpdb->posts p " .
                    "INNER JOIN $wpdb->postmeta pm ON (p.ID=pm.post_id AND pm.meta_key='external_variation_id' AND pm.meta_value IN ({$preparedIn})) " .
                    ...
                    "WHERE post_parent = %d and post_type='product_variation' ORDER BY post_date DESC",
                    $product_id),
                ARRAY_A);
        ...
        foreach ($variations['variations'] as $variation) {
            ...
            //we should import variant for simple product in order to have this data in database
            if (!$old_vid && (!$is_update || $on_new_variation_appearance === 'add' || $isSimpleProduct)) {

                // this is a new variant, therefore do NOT CHECK for $is_price_changed and $is_stock_changed
        ...
        if ($step === false || $step === 'variations#sync') {
            ...
            // for simple variations attributes, update attributes values (save only used values)
            $need_update = false;
    ...
    public function get_sorted_products_ids($sort_type, $ids_count, $compare = false, $untrash = false)
    {
        ...
        if ($compare) {
            ...
            //todo: need refactor this code because next 2 queries look very similar
            if (!empty($operation) && !empty($value)) {
                $querySql =   "SELECT p.ID FROM $wpdb->posts p " .
                              "INNER JOIN $wpdb->postmeta pm1 ON (pm1.post_id=p.ID AND pm1.meta_key='_a2w_external_id') " .
                              "LEFT JOIN $wpdb->postmeta pm2 ON (pm2.post_id=p.ID AND pm2.meta_key=%s) " .
                              "LEFT JOIN $wpdb->postmeta pm3 ON (pm3.post_id=p.ID AND pm3.meta_key=%s) " .
                              "WHERE p.post_type = 'product' AND pm1.meta_value != '' " .
                                    "AND (p.post_status != 'trash' OR NOT pm3.meta_key IS NULL) " .
                                    "AND (pm2.meta_value is null OR pm2.meta_value $operation %s) " .
                              "ORDER BY pm2.meta_value LIMIT %d";

                $query = $wpdb->prepare(
                    // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                    $querySql, $sort_type, $autoremove_meta_key, $value, $ids_count
                );
            }
        } else {
            $querySql = "SELECT p.ID FROM $wpdb->posts p " .
            ...
            $query = $wpdb->prepare(
                // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                $querySql, $sort_type, $autoremove_meta_key, $ids_count
            );
        }
    ...
    public function get_products_ids($page, $products_per_page): array
    {
        $result = [];
        $ids0 = get_posts([
            'post_type' => 'product',
    ...
    {
        global $wpdb;
        $query = "SELECT count(DISTINCT post_id) from $wpdb->postmeta " .
                 "WHERE meta_key = '_a2w_external_id' AND meta_value != '' AND meta_value IS NOT NULL";
